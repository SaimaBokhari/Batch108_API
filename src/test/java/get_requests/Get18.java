...
public class Get18 extends GMIBankBaseUrl {
    ...
    /* 1) Create basic authorisation on postman .. not recommended

       2) Create bearer token on intelliJ
       swagger document => user_jwt (json web token)_controller
       Post request at the endpoint https://www.gmibank.com//api/authenticate  + body + send request
    ...
    public void get18(){
        ...
        // create empty array list for accounts
...
//                spec(spec).
//                headers("Authorization","Bearer eyJhbGciOiJIUzUxMiJ9.eyJzdWIiOiJqb2huX2RvZSIsImF1dGgiOiJST0xFX0FETUlOIiwiZXhwIjoxNjcyMzMwNDc3fQ.OmqC3n1C-umcMCECLKEHqEq19e5tgZ1vB2CuZmaRIBNFSdW6cJH9xMoMRi5JijkxFeWePnFmJCx5ZzAVOpfPyw").
//                when().
//                get("/{first}/{second}");
//        response.prettyPrint();

        // this token stops working every 24 hours ... so we need more dynamic way ...
        // we call generateToken() from the respective class..see following


        Response response = given().
                spec(spec).
                headers("Authorization","Bearer "+ generateToken()).
                when().    // this token stops working every 24 hours
                        get("/{first}/{second}");

        response.prettyPrint();

        // Do assertion
        // de-serialisation with Object Mapper

        Customer actualData =  JsonUtils.convertJsonToJavaObject(response.asString(), Customer.class);
